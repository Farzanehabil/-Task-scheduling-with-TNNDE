...
//6 parameter TNNDE
    ...
    public  Runsched_GOCJ(int pop, int iter,int vm, int task) {
        this.pop= pop;
        this.max_iter = iter;
        this.num_vm= vm;
        this.num_task = task;
    ...
    for(int k=0; k< M; k++){
        ...
        for(int i=0 ; i<N; i++){  
           ...
           System.out.println("Execute simulatin with ------------------>> : "+cloudletCount );
            ...
            String datasetFilePath = "D:\\dataset\\GoCJ Google Cloud Jobs Dataset\\GoCJ_Dataset_" + cloudletCounts[i] + ".txt";  
            System.out.println("Processing file: " + datasetFilePath);  
            List<Cloudlet> cloudletList = loadJobsFromDataset(datasetFilePath, broker.getId());  
    ...
    //---------start MSDE---------------------------------------------
            ...
            cloudletList = loadJobsFromDataset(datasetFilePath, broker.getId());  
    ...
    //-------------start HGHHC Algorithm--------------------
            System.out.println("------HGHHC Algorithm-----------------");
            CloudSim.init(num_user, calendar, trace_flag);  
            datacenter0 = createDatacenter("Datacenter_0");  
            broker = createBroker();  
            brokerId = broker.getId();
    ...
    private static List<Cloudlet> loadJobsFromDataset(String filePath, int brokerId) {  
        List<Cloudlet> cloudletList = new ArrayList<>();  
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {  
            String line;  
            while ((line = br.readLine()) != null) {  
                int jobSizeInMI = Integer.parseInt(line.trim());  

                // ایجاد Cloudlet  
                Cloudlet cloudlet = new Cloudlet(  
                    cloudletList.size(),           // id  
                    jobSizeInMI,                   // length  
                    1,                             // pesNumber  
                    jobSizeInMI,                   // fileSize  
                    jobSizeInMI,                   // outputSize  
                    new UtilizationModelFull(),    // Utilization Model for CPU  
                    new UtilizationModelFull(),    // Utilization Model for RAM  
                    new UtilizationModelFull()     // Utilization Model for IO  
                );  

                cloudlet.setUserId(brokerId); // تعیین شناسه کاربر  
