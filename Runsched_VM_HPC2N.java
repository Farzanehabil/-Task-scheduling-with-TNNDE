 ...
 * @author farzaneh
 */
public class Runsched_VM_HPC2N {
...
//6 parameter TNNDE
    ...
    public  Runsched_VM_HPC2N(int pop, int iter,int vm, int task) {
        this.pop= pop;
        this.max_iter = iter;
        this.num_vm = vm;
        this.num_task = task;
    ...
    public  void Execute()throws Exception{    
        ...
        List<Cloudlet> originalCloudletList = CreatCloudlet.createCloudlet_HP(brokerId, num_task); 
        
       
    
        int[] vmn_Counts = new int[N];    
        for(int k=0; k< M; k++){
            int vmn_Count = 20;
            for(int i=0 ; i<N; i++){  
               vmn_Counts[i]= vmn_Count;
               System.out.println("Execute simulatin with VM ------------------>> : "+ num_vm );
    ...
    //----------start FPGWO---------------------------------------------
            ...
            List<Cloudlet> cloudletList = new ArrayList<>(originalCloudletList.subList(0, num_task));
    ...
    //---------start MSDE---------------------------------------------
            ...
            broker.getVmList().clear();  
            broker.submitVmList(vmList);  
             cloudletList = new ArrayList<>(originalCloudletList.subList(0, num_task));
    ...
    //-------------End TNNDE-----------------------------
        ...
        average();
    //----------------print of Result-----------------------------------------------------------
        print.print("Resource utilization",vmn_Counts,RU_fpa,RU_msa,RU_empa,RU_hghhc,RU_ibwc, RU_tnnde );
        print.print("Makespan",vmn_Counts,MAK_fpa,MAK_msa,MAK_empa,MAK_hghhc,MAK_ibwc,MAK_tnnde);
        print.print("throughput",vmn_Counts,TH_fpa,TH_msa,TH_empa,TH_hghhc,TH_ibwc, TH_tnnde);
        print.print("Degree of imbalance",vmn_Counts,deg_fpa,deg_msa,deg_empa,deg_hghhc, deg_ibwc, deg_tnnde);
        
        
        
    //---------------------plot of Result----------------------------------------------------------------    
        plat.plot_VM("Resource utilization(%)",vmn_Counts,RU_fpa,RU_msa,RU_empa,RU_hghhc,RU_ibwc, RU_tnnde );
        plat.plot_VM("throughput",vmn_Counts,TH_fpa,TH_msa,TH_empa,TH_hghhc,TH_ibwc, TH_tnnde);
        plat.plot_VM("Degree of imbalance",vmn_Counts,deg_fpa,deg_msa,deg_empa,deg_hghhc, deg_ibwc, deg_tnnde);
        plat.plot_VM("Makespan(Seconds)",vmn_Counts,MAK_fpa,MAK_msa,MAK_empa,MAK_hghhc,MAK_ibwc,MAK_tnnde);
    }
    ...
    private static List<Vm> createVM(int brokerId, int numberOfVms) {  
        ...
        int v2= numberOfVms/2;
        int v3= numberOfVms/2;
