...
public class TNNDE {
        ...
        private List<Vm> vmList; 
         double[] xglobal;
         double globalfit;
         double[] best_fit_iter;
        
    public TNNDE(int populationSize, int maxIterations, List<Cloudlet> cloudletList, List<Vm> vmList){
...
//             System.out.println("weight initalize----------------------------------------------------------------");
//             weight_cal();
    }    
    private double[][] sumation_weight(double[][] w){
        ...
        for(int j=0; j< Npop; j++){
           ...
           // مرحله 2: نرمال‌سازی مقادیر برای اینکه مجموع برابر با 1 باشد  
            if (sum > 0) { // اطمینان از اینکه مقدار جمع کلاً صفر نباشد  
                for (int i = 0; i < Npop; i++) {  
                    w[i][j] /= sum;  
                }  
            } else { 
...
public int[] execute_TLNND(){ 
   ...
   //process population with NNA for worst half ----------------------------------------
        ...
        for(int i=0; i<Npop; i++){                
            ...
            }else{  //Transfer Function Operator
                   for(int j=0; j<dim; j++){
            ...
            for(int i=0; i< Npop; i++){
                cost_worst[i]= Calculator.calculateFitness(worst[i], cloudletList, vmList);
            }
 ...
 //process population with tlbo for half best
        ...
        for(int i=0; i<pop/2; i++){
            ...
            fit_new_sol= Calculator.calculateFitness(new_sol, cloudletList, vmList);
         ...
         //    Learner phase
            for(int i=0; i<pop/2 ; i++){
                ...
                double fit_rnd1= Calculator.calculateFitness(xrand1, cloudletList, vmList);
                double fit_rnd2= Calculator.calculateFitness(xrand2, cloudletList, vmList);
                double[] new_sol= new double[dim];
                ...
                new_sol = check_bound(new_sol);
                double fit_new_sol= Calculator.calculateFitness(new_sol, cloudletList, vmList);
                if(fit_new_sol < cost_best[i]){
                    best[i] = new_sol.clone();
                    cost_best[i]= fit_new_sol;
                }
    ...
    //===========================================================================
        for(int i=0; i< Npop; i++){
            ...
            if(cost_best[i]< cost[i+Npop]){
                x[i+Npop] = best[i].clone();
                cost[i+Npop]= cost_best[i];
            }else{
                best[i]= crossover(best[i], xglobal);     check_bound(best[i]);
                cost_best[i]= Calculator.calculateFitness(best[i], cloudletList, vmList);
        ...
        best_fit_iter[t]= globalfit;  
     }
        
        return  Arrays.stream(xglobal).map(Math::round).mapToInt((xx) -> (int) xx).toArray();
   }
